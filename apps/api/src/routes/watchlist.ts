...
import { CreateWatchlistSchema, UpdateWatchlistSchema, FitAnalysisSchema } from '@parcours/shared'
...
const listRoute = createRoute({
  ...
  responses: {
    200: {
      content: { 'application/json': { schema: z.array(WatchlistResponseSchema) } },
...
watchlistRoutes.openapi(getRoute, async (c) => {
  ...
  const [item] = await db
    .select()
    .from(watchlist)
    .where(and(eq(watchlist.id, id), eq(watchlist.userId, user.id), isNull(watchlist.deletedAt)))
    .limit(1)
...
const createRoute_ = createRoute({
  ...
  path: '/',
  request: {
    body: { content: { 'application/json': { schema: CreateWatchlistSchema } }, required: true },
  },
  responses: {
    201: {
      content: { 'application/json': { schema: WatchlistResponseSchema } },
...
const updateRoute = createRoute({
  ...
  request: {
    ...
    body: { content: { 'application/json': { schema: UpdateWatchlistSchema } }, required: true },
  },
  responses: {
    200: {
      content: { 'application/json': { schema: WatchlistResponseSchema } },
...
watchlistRoutes.openapi(analyzeRoute, async (c) => {
  ...
  const result = streamAnalysis(prompt, { abortSignal: AbortSignal.timeout(30_000) })
...
const applyRoute = createRoute({
  ...
  responses: {
    200: {
      content: {
        'application/json': { schema: z.object({ applicationId: z.string().uuid() }) },
      },
      description: 'Applied',
    },
    404: {
