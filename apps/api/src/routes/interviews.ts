...
const listRoute = createRoute({
  ...
  responses: {
    200: {
      content: { 'application/json': { schema: z.array(InterviewResponseSchema) } },
...
interviewRoutes.openapi(listRoute, async (c) => {
  ...
  const data = await db
    .select()
    .from(interviews)
    .where(and(eq(interviews.applicationId, appId), isNull(interviews.deletedAt)))
...
const createRoute_ = createRoute({
  ...
  request: {
    ...
    body: { content: { 'application/json': { schema: CreateInterviewSchema } }, required: true },
  },
  responses: {
    201: {
...
const updateRoute = createRoute({
  ...
  request: {
    ...
    body: { content: { 'application/json': { schema: UpdateInterviewSchema } }, required: true },
  },
  responses: {
    200: {
      content: { 'application/json': { schema: InterviewResponseSchema } },
...
interviewRoutes.openapi(updateRoute, async (c) => {
  ...
  const [existing] = await db
    .select()
    .from(interviews)
    .where(and(eq(interviews.id, id), eq(interviews.applicationId, appId), isNull(interviews.deletedAt)))
    .limit(1)
...
const deleteRoute = createRoute({
  method: 'delete',
  path: '/:appId/interviews/:id',
  request: { params: z.object({ appId: z.string().uuid(), id: z.string().uuid() }) },
  responses: {
...
interviewRoutes.openapi(deleteRoute, async (c) => {
  ...
  await db.update(interviews).set({ deletedAt: new Date() }).where(eq(interviews.id, id))
