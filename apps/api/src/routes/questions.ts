...
import { CreateQuestionSchema, UpdateQuestionSchema, LinkQuestionSchema } from '@parcours/shared'
...
const listRoute = createRoute({
  ...
  responses: {
    200: {
      content: { 'application/json': { schema: z.array(QuestionResponseSchema) } },
...
questionRoutes.openapi(getRoute, async (c) => {
  ...
  const [q] = await db
    .select()
    .from(questions)
    .where(and(eq(questions.id, id), eq(questions.userId, user.id), isNull(questions.deletedAt)))
    .limit(1)
...
const createRoute_ = createRoute({
  ...
  path: '/',
  request: {
    body: { content: { 'application/json': { schema: CreateQuestionSchema } }, required: true },
  },
  responses: {
    201: {
      content: { 'application/json': { schema: QuestionResponseSchema } },
...
const updateRoute = createRoute({
  ...
  request: {
    ...
    body: { content: { 'application/json': { schema: UpdateQuestionSchema } }, required: true },
  },
  responses: {
    200: {
      content: { 'application/json': { schema: QuestionResponseSchema } },
...
questionRoutes.openapi(updateRoute, async (c) => {
  ...
  const [updated] = await db
    ...
    .set({
      ...
      sourceCompany: body.sourceCompany !== undefined ? body.sourceCompany : existing.sourceCompany,
    })
...
const linkRoute = createRoute({
  ...
  request: {
    params: z.object({ id: z.string().uuid() }),
    body: { content: { 'application/json': { schema: LinkQuestionSchema } }, required: true },
  },
  responses: {
    200: {
...
questionRoutes.openapi(linkRoute, async (c) => {
  ...
  try {
    await db.insert(applicationQuestions).values({ applicationId, questionId: id })
