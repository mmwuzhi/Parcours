...
import { useApplications, useCreateApplication, useUpdateApplication } from '@/hooks/use-applications'
...
export function KanbanBoard() {
  ...
  return (
    <div style={{ padding: '24px 28px', height: '100%' }}>
      ...
      {/* Active columns */}
      <div style={{ display: 'flex', gap: 12, overflowX: 'auto', paddingBottom: 8, minHeight: 0 }}>
        {ACTIVE_COLS.map(col => (
          <Column
            key={col.status}
            label={col.label}
      ...
      <div style={{ marginTop: 20 }}>
        ...
        >
          {showTerminal ? '▾' : '▸'} {' '}
          Closed ({byStatus('REJECTED').length + byStatus('WITHDRAWN').length})
        </button>
      ...
      {showModal && (
        <Modal onClose={() => { setShowModal(false); reset() }}>
          <h2 style={{ fontSize: 16, fontWeight: 700, marginBottom: 20 }}>Add application</h2>
          <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <FormField label="Company *" error={errors.company?.message}>
              <input {...register('company')} placeholder="Acme Corp" style={inputStyle} autoFocus />
            </FormField>
            <FormField label="Role *" error={errors.role?.message}>
              <input {...register('role')} placeholder="Software Engineer" style={inputStyle} />
            </FormField>
            <FormField label="Salary range">
              <input {...register('salaryRange')} placeholder="e.g. $120k–$150k" style={inputStyle} />
            </FormField>
...
function Column({
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          style={{
            fontSize: 11,
            fontWeight: 600,
            color: apps.length > 0 ? 'var(--primary-light)' : 'var(--text-muted)',
            background: apps.length > 0 ? 'var(--primary-subtle)' : 'transparent',
            padding: '1px 7px',
            borderRadius: 100,
          }}
        >
      ...
      >
        {apps.length === 0 && (
          <div style={{ fontSize: 12, color: 'var(--text-muted)', padding: '8px 4px', textAlign: 'center' }}>
            No applications
          </div>
        )}
        {apps.map(app => (
