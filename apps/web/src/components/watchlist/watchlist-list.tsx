...
import { useWatchlist, useCreateWatchlist, useDeleteWatchlist } from '@/hooks/use-watchlist'
...
export function WatchlistList() {
  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      ...
      >
        {items.map(item => (
          <WatchlistCard key={item.id} item={item} onDelete={() => handleDelete(item.id)} />
        ))}
      </div>

      {/* Add modal */}
      {showModal && (
        <Modal onClose={() => { setShowModal(false); reset() }}>
          <h2 style={{ fontSize: 16, fontWeight: 700, marginBottom: 20 }}>Add to watchlist</h2>
          <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <FormField label="Company *" error={errors.company?.message}>
              <input {...register('company')} placeholder="Acme Corp" style={inputStyle} autoFocus />
            </FormField>
            <FormField label="Role *" error={errors.role?.message}>
              <input {...register('role')} placeholder="Staff Engineer" style={inputStyle} />
            </FormField>
            <FormField label="Job posting URL" error={errors.jdUrl?.message}>
              <input {...register('jdUrl')} placeholder="https://…" style={inputStyle} />
            </FormField>
            <FormField label="Salary range">
              <input {...register('salaryRange')} placeholder="e.g. $150k–$200k" style={inputStyle} />
            </FormField>
            <FormField label="Notes">
              <textarea {...register('notes')} placeholder="Why you're watching…" rows={3} style={{ ...inputStyle, resize: 'vertical' }} />
            </FormField>
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>{item.role}</div>
        </div>
        <button
          onClick={onDelete}
      ...
      {item.salaryRange && (
        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{item.salaryRange}</div>
      )}
      ...
      {score != null && (
        <div
          style={{
            ...
            fontSize: 11,
            fontWeight: 600,
            background: score >= 70 ? 'oklch(0.92 0.08 145)' : score >= 50 ? 'oklch(0.93 0.08 75)' : 'oklch(0.93 0.05 25)',
            color: score >= 70 ? 'oklch(0.35 0.14 145)' : score >= 50 ? 'oklch(0.42 0.15 75)' : 'oklch(0.40 0.16 25)',
            padding: '2px 8px',
            borderRadius: 100,
          }}
        >
